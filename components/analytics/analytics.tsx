...
// ─── Recharts tooltip payload shape (what actually arrives at runtime) ───────
...
// ─── Data ────────────────────────────────────────────────────────────────────
interface AreaData { name: string; revenue: number; }
...
interface BarData { name: string; clicks: number; conv: number; }
...
// ─── Tooltip components (typed against our own interface, cast at usage) ──────
function RevenueTooltip({ active, payload, label }: CustomTooltipProps): React.ReactElement | null {
    ...
    <div className="bg-white border border-[#006E74]/20 rounded-xl px-3 py-2 shadow-lg text-xs">
      ...
      {payload.map((p, i) => (
        <p key={i} className="font-semibold" style={{ color: p.color }}>
          {p.dataKey === 'clicks' ? 'Clicks' : 'Conversions'}: {Number(p.value).toLocaleString()}
        </p>
      ))}
    </div>
  );
...
// Single cast helper — keeps JSX clean, isolates the suppression to one place
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const tooltipContent = (fn: (props: CustomTooltipProps) => React.ReactElement | null) => fn as any;
      ...
      <div className="max-w-screen-2xl mx-auto px-3 xs:px-4 md:px-8 py-3 xs:py-4 md:py-8 pb-24 md:pb-8 space-y-3 xs:space-y-4 sm:space-y-5">
            ...
            <div className="h-36 xs:h-44 sm:h-60 lg:h-72 w-full min-w-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={areaData} margin={{ top: 4, right: 4, left: -15, bottom: 0 }}>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#6B5E5E', fontSize: 9 }} dy={6} />
                  <YAxis axisLine={false} tickLine={false} width={30} tick={{ fill: '#6B5E5E', fontSize: 9 }} />
                  {/* ✅ tooltipContent() cast isolates the any to one helper */}
                  <Tooltip content={tooltipContent(RevenueTooltip)} />
                  <Area type="monotone" dataKey="revenue" stroke="#F35D2C" strokeWidth={2} fillOpacity={1} fill="url(#colorRev)" dot={false} activeDot={{ r: 4 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="h-36 xs:h-44 sm:h-60 lg:h-72 w-full min-w-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barData} margin={{ top: 4, right: 4, left: -15, bottom: 0 }} barGap={2}>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#6B5E5E', fontSize: 9 }} dy={6} />
                  <YAxis axisLine={false} tickLine={false} width={30} tick={{ fill: '#6B5E5E', fontSize: 9 }} />
                  <Tooltip content={tooltipContent(BarTooltip)} />
        ...
        <section className="bg-white border border-[#006E74]/10 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-[#006E74]/10">
            {campaigns.map(c => (
              <div key={c.rank} className="flex flex-col xs:flex-row xs:items-center justify-between px-3 xs:px-4 sm:px-5 py-3 hover:bg-[#006E74]/5 transition-colors cursor-pointer gap-2">
